...
export async function getTurnsForAdminDashboard() {
  try {
    ...
    for (const turno of enrichedTurnos) {
      ...
      // IMPORTANTE: los turnos pendientes se gestionan primero desde "Próximos a Confirmar".
      // Una vez confirmados, recién pasan a "Turnos del Día" o "Turnos del Mes".
      ...
      // IMPORTANTE: todos los turnos del día (incluyendo finalizados y en curso) deben seguir
...
export async function getTurnosForReports() {
  try {
    const turnosSnapshot = await db.collectionGroup('turnos').get();
    ...
    const enrichedTurnos = await Promise.all(turnosSnapshot.docs.map(async (doc) => {
      const turnoData = doc.data();
      const userId = turnoData.clienteId;
      const mascotaId = turnoData.mascotaId;

      let serializableUser = { id: userId, nombre: 'Usuario', apellido: 'Eliminado', email: 'N/A' };
      let serializableMascota = { id: mascotaId, nombre: 'Mascota Eliminada', especie: 'N/A', raza: 'N/A' };

      if (userId) {
        if (usersCache.has(userId)) serializableUser = usersCache.get(userId);
        else {
          const userDoc = await db.collection('users').doc(userId).get();
          if (userDoc.exists) {
            const userData = userDoc.data();
      ...
      if (userId && mascotaId) {
        const mascotaCacheKey = `${userId}-${mascotaId}`;
        if (mascotasCache.has(mascotaCacheKey)) serializableMascota = mascotasCache.get(mascotaCacheKey);
        else {
          const mascotaDoc = await db.collection('users').doc(userId).collection('mascotas').doc(mascotaId).get();
          if (mascotaDoc.exists) {
            const mascotaData = mascotaDoc.data();
      ...
      return {
        ...
        userId,
        mascotaId,
        tipo: turnoData.tipo || 'clinica',
        servicioNombre: turnoData.servicioNombre || 'Sin servicio',
        estado: turnoData.estado || 'pendiente',
        necesitaTraslado: Boolean(turnoData.necesitaTraslado),
        fecha: toISOStringOrNull(turnoData.fecha),
        precio: typeof turnoData.precio !== 'undefined' ? turnoData.precio : null,
        metodoPago: turnoData.metodoPago || turnoData.metodoDePago || '',
        motivoCancelacion: turnoData.motivoCancelacion || '',
        comentario: turnoData.comentario || '',
        medicamentosSuministrados: turnoData.medicamentosSuministrados || [],
        user: serializableUser,
    ...
    const sortedTurnos = enrichedTurnos.filter((t) => t.fecha).sort((a, b) => new Date(b.fecha) - new Date(a.fecha));
    return { success: true, data: sortedTurnos };
  } catch (error) {
    console.error('Error en getTurnosForReports:', error);
    return { success: false, error: `Error del servidor: ${error.message}.` };
  }
}
